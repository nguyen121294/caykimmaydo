...
export default function AdminManagementPage() {
  ...
  const handleResetPasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        toast.success(`Đã đổi mật khẩu thành công cho người dùng: ${selectedUserForReset.email}`);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 -m-4 md:-m-6 lg:-m-8 p-4 lg:p-8">
          ...
          <div className="lg:col-span-5">
            <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl space-y-5">
              <div className="flex items-center gap-3 border-b border-slate-800 pb-4">
                ...
                <div>
                  <h2 className="text-base font-semibold text-white">Tạo Tài Khoản Mới</h2>
                  <p className="text-xs text-slate-400">Tạo tài khoản & mật khẩu cho nhân viên mới</p>
                </div>
              </div>

              <form onSubmit={handleCreateUser} className="space-y-4">
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5 block flex items-center gap-1.5">
                    <User size={14} className="text-indigo-400" /> Họ & Tên
                  </label>
                  <input
                    type="text"
                    value={name}
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 transition text-sm"
                  />
                </div>

                <div>
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5 block flex items-center gap-1.5">
                    <Mail size={14} className="text-indigo-400" /> Email Đăng Nhập
                  </label>
                  <input
                    type="email"
                    value={email}
                ...
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5 block flex items-center gap-1.5">
                    <KeyRound size={14} className="text-indigo-400" /> Mật Khẩu
                  </label>
                  <input
                    type="text"
                    value={password}
                    ...
                    placeholder="••••••••"
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 transition text-sm font-mono"
                    required
                  />
                </div>
                ...
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1.5 block flex items-center gap-1.5">
                    <ShieldCheck size={14} className="text-indigo-400" /> Phân Quyền (Role)
                  </label>
                  <select
                    value={role}
                    onChange={(e) => setRole(e.target.value)}
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 focus:outline-none focus:border-indigo-500 transition text-sm"
                  >
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-semibold text-white">Danh Sách Người Dùng ({users.length})</h2>
                    <p className="text-xs text-slate-400">Các tài khoản đã được cấp quyền trong hệ thống</p>
                  </div>
                </div>
                <button
                  onClick={fetchUsers}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 transition text-xs"
                />
              </div>
                    ...
                    <tbody className="divide-y divide-slate-800/60 text-slate-200">
                      {filteredUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-slate-800/40 transition">
                          ...
                          <td className="p-3 text-indigo-400">{u.email}</td>
                          <td className="p-3">
                            <span
                          ...
                          <td className="p-3">
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/30 text-indigo-300 transition flex items-center gap-1 text-[11px]"
      ...
      {selectedUserForReset && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl relative space-y-4 text-slate-100">
            <button
            ...
            <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
              ...
              <div>
                <h3 className="text-base font-semibold text-white">Đổi Mật Khẩu Người Dùng</h3>
                <p className="text-xs text-indigo-400 font-mono">{selectedUserForReset.email}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleResetPasswordSubmit} className="space-y-4 pt-2">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 focus:outline-none focus:border-indigo-500 transition text-sm font-mono"
                  required
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs transition shadow-lg shadow-indigo-600/25 disabled:opacity-50 flex items-center gap-2"
                >
